...
//const API_KEY=  FirebaseConstant.configFb.apiKey;
...
class Videos extends Component {
	...
	getVideoDetails(q) {
		...
		this.setState({videoTitle:obj.title,
					   videoDescription:obj.description,
					   videoThumbnail:obj.imageUrl
					   });					  
	...
	submitFrm(e) {
		...
		var url = FirebaseConstant.basePath + '/list/' + this.props.match.params.list + '/videos/';
		var unique_id = firebaseDatabase.ref(url).push(obj).key;// save into firebase db
		...
		//k is categories id, value is true
		for (let k in this.state.categories){
			...
			if(tmp.length ===2 ){
				subcat = tmp[1];
				firebaseDatabase.ref(catUrl).child(cat).child('subcategories').child(subcat).child('videos').child(unique_id).set(current);	//url for subcat	
			
			} else {			
				firebaseDatabase.ref(catUrl).child(cat).child('videos').child(unique_id).set(current);// url for cat
		    }
			
		
		}

		this.setState({error:'Video Successfully added in the list', 
					  videoInput:'',
					  videoInputId:'',
					  videoTitle:'',
					  videoDescription:'',
						...
						{myArrayConverted && 
							...
							{myArrayConverted.map((value, index)=>{
								...
								return <li key={index} className="list-group-item">
										<div className="row">
										<div className="col-md-4"><img src={value.videoThumbnail} className="img-resposive img-thumbnail "alt="" /></div>
										<div className="col-md-8"><a href={url} target="_blank">{value.videoTitle}</a></div>
										</div>								
								       </li>															
							})
							}			
...
export default Videos;// JavaScript Document